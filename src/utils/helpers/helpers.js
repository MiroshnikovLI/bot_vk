...
function findAdminKeyByPartialMatch(searchText, obj, path = []) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    // Если значение - объект, рекурсивно обходим его (кроме геттеров PARENTS)
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      // Пропускаем геттеры PARENTS (они функции)
      const hasGetters = Object.values(value).some(v => typeof v === 'function');
      if (!hasGetters) {
        const result = findAdminKeyByPartialMatch(searchText, value, currentPath);
