...
async function getAllPvzs() {
  try {
    const result = await query(
      `SELECT * FROM pvz ORDER BY pvz_id `,
    );
...
async function getAllActivePvzs(active = true) {
    try {
    const result = await query(
      `SELECT * FROM pvz WHERE is_active = $1 ORDER BY pvz_id `, [active]
    );
...
async function setActivePvzFromDb(pvzId, active) {
  try {
    const result = await query(
      `UPDATE pvz SET is_active = $1 WHERE pvz_id = $2
      RETURNING *`,
    ...
    return { success: true, message: "ПВЗ был удален с активных", data: result.rows[0] };
  } catch (error) {
    return { success: false, message: error.message };
  }
}
...
async function addReplacementDb(userId, replacementId) {
  try {
    ...
    if (result.rows.length === 0) {
      return { success: false, message: "Не удалось привязать сменщика", data: result.rows };
    }
    return { success: true, message: "Сменщик привязан", data: result.rows };
  } catch (error) {
    if (error.code === "23505") {
